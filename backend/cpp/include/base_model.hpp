...
class BaseModel {
   public:
    ...
    virtual uint32_t* forward(const Tensor<uint32_t>* input, ThreadPool& thread_pool, KVCacheBase* kv_cache,
                              size_t top_k, float temperature, float top_p, curandState* d_states = nullptr) = 0;
    virtual uint32_t* prefill(const Tensor<uint32_t>* input, ThreadPool& thread_pool, KVCacheBase* kv_cache,
                              size_t top_k, float temperature, float top_p, curandState* d_states = nullptr) = 0;
    ...
    void print_device_info() const {
        std::cout << "模型当前设备: " << (device() == Device::CUDA ? "CUDA" : "CPU") << std::endl;
    }
};
