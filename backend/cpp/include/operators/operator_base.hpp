...
class RopeOperator : public OperatorBase {
   public:
    ...
    // RoPE算子实现 - 使用一重指针
    virtual void operator()(Tensor<T>* tensor, size_t offset, float theta, cudaStream_t stream = nullptr) = 0;

    // 获取算子类型
    OperatorType type() const override {
        return OperatorType::ROPE;
    }
...
class RmsNormOperator : public OperatorBase {
   public:
    ...
    // RMS Norm算子实现 - 使用一重指针
    virtual void operator()(Tensor<T>* output, Tensor<T>* input, Tensor<T>* weight, float eps,
                            cudaStream_t stream = nullptr) = 0;

    // 获取算子类型
    OperatorType type() const override {
        return OperatorType::RMS_NORM;
    }
...
class AddOperator : public OperatorBase {
   public:
    ...
    // Add算子实现 - 使用一重指针
    virtual void operator()(Tensor<T>* output, Tensor<T>* input_a, Tensor<T>* input_b,
                            cudaStream_t stream = nullptr) = 0;
...
class SiluOperator : public OperatorBase {
   public:
    ...
    // SiLU算子实现 - 使用一重指针
    virtual void operator()(Tensor<T>* output, Tensor<T>* input, cudaStream_t stream = nullptr) = 0;
...
class MatmulOperator : public OperatorBase {
   public:
    ...
    virtual void operator()(Tensor<T>* output, Tensor<T>* input, const WeightTensor<T>& weight,
                            const Tensor<T>* bias = nullptr, cudaStream_t stream = nullptr) = 0;
...
class OperatorRegistry {
   public:
    ...
    // 注册算子
    void registerOperator(OperatorType type, OperatorPlatform platform, std::shared_ptr<OperatorBase> op) {
    ...
    std::shared_ptr<OpType> getOperator(OperatorType type, OperatorPlatform platform) {
        std::string key = getKey(type, platform);
        if (operators_.find(key) == operators_.end()) {
            return nullptr;
        }
