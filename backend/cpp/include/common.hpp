 ...
 */
template <typename T>
inline void debugPrintTensor(const Tensor<T>& tensor,
 ...
 * @brief 同步当前CUDA设备
 */
inline void sync_cuda_device() { CUDA_CHECK(cudaDeviceSynchronize()); }

/**
 * @brief 重置当前CUDA设备
 */
inline void reset_cuda_device() { CUDA_CHECK(cudaDeviceReset()); }
 ...
 */
template <typename T>
inline void print_shape(const std::vector<T>& shape,
                        const std::string& name = "") {
  if (!name.empty()) {
    std::cout << name << " shape: ";
  }
 ...
 */
template <typename T>
inline bool saveTensorToFile(const Tensor<T>& tensor,
  ...
  if (tensor.device() == Device::CUDA) {
    ...
    // 拷贝数据
    cudaError_t err =
        cudaMemcpy(host_buffer.data(), tensor.data_ptr(),
                   total_elements * sizeof(T), cudaMemcpyDeviceToHost);
    if (err != cudaSuccess) {
      std::cerr << "从GPU复制数据失败: " << cudaGetErrorString(err)
                << std::endl;
      fclose(file);
      return false;
    }
 ...
 */
template <>
inline bool saveTensorToFile<__nv_bfloat16>(const Tensor<__nv_bfloat16>& tensor,
                                            const std::string& filename,
  ...
  if (tensor.device() == Device::CUDA) {
    ...
    // 拷贝数据
    cudaError_t err = cudaMemcpy(bf16_buffer.data(), tensor.data_ptr(),
                                 total_elements * sizeof(__nv_bfloat16),
 ...
 */
template <>
inline bool saveTensorToFile<__half>(const Tensor<__half>& tensor,
                                     const std::string& filename,
                                     const std::string& mode) {
  // 创建输出目录（如果不存在）
  std::string dir_path = filename.substr(0, filename.find_last_of('/'));
  ...
  if (tensor.device() == Device::CUDA) {
    ...
    // 拷贝数据
    cudaError_t err =
        cudaMemcpy(half_buffer.data(), tensor.data_ptr(),
                   total_elements * sizeof(__half), cudaMemcpyDeviceToHost);
    if (err != cudaSuccess) {
      std::cerr << "从GPU复制数据失败: " << cudaGetErrorString(err)
                << std::endl;
      fclose(file);
