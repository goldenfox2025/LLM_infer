...
class CudaMemoryPool {
 public:
  ...
  ~CudaMemoryPool() {
    ...
    if (driver_available) {
      ...
      if (driver_available && prefill_buffer_ != nullptr) {
        ...
        if (err != cudaSuccess) {
          if (err == cudaErrorCudartUnloading) driver_available = false;
        } else {
          prefill_buffer_ = nullptr;
          prefill_buffer_size_ = 0;
          prefill_buffer_used_ = 0;
  ...
  void* try_allocate_from_prefill(size_t size, size_t aligned_size,
                                  bool is_prefill_context) {
    ...
    if (prefill_buffer_size_ < prefill_max_size_) {
      size_t new_size =
          std::min(prefill_buffer_size_ * 3 / 2, prefill_max_size_);
      ...
      if (prefill_buffer_used_ > 0) {
        cudaError_t copy_err =
            cudaMemcpy(new_buffer, prefill_buffer_, prefill_buffer_used_,
  ...
  void* allocate_tagged(const std::string& tag, size_t size,
                        bool is_prefill_request = false) {
    if (tag.empty()) {
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    if (is_shutting_down_) return nullptr;
    ...
    if (actual_is_prefill_request) {
      new_ptr = try_allocate_from_prefill(size, aligned_size, true);
      if (new_ptr) {
        tagged_memory_[tag] = new_ptr;
        memory_tags_[new_ptr] = tag;
        tagged_memory_sizes_[tag] = aligned_size;
        tagged_memory_active_[tag] = true;
        return new_ptr;
      }
  ...
  bool enable_prefill_mode(size_t initial_size = 48 * 1024 * 1024,
    ...
    if (err != cudaSuccess) {
      std::cerr << "CudaMemoryPool: Error - Failed to allocate prefill buffer: "
                << (adjusted_initial_size / (1024.0 * 1024.0))
                << " MB, Error: " << cudaGetErrorString(err) << std::endl;
      prefill_buffer_ = nullptr;
      prefill_buffer_size_ = 0;
      prefill_buffer_used_ = 0;
      is_prefill_mode_ = false;
    ...
    std::cerr << "CudaMemoryPool: Prefill mode enabled. Initial: "
              << (prefill_buffer_size_ / (1024.0 * 1024.0))
              << " MB, Max: " << (prefill_max_size_ / (1024.0 * 1024.0))
              << " MB" << std::endl;
    return true;
  }
    ...
    if (mem_err == cudaSuccess) {
      if (requested_size > free_memory * 0.8) {
        ...
        std::cerr << "CudaMemoryPool: Warning - Requested prefill size ("
                  ...
                  << "MB) is too large. Adjusted to: "
                  << (adjusted_size / (1024.0 * 1024.0)) << " MB (70% of free "
                  << (free_memory / (1024.0 * 1024.0)) << "MB)" << std::endl;
 ...
 private:
  void enable_prefill_mode_internal(size_t initial_size = 48 * 1024 * 1024,
                                    size_t max_size = 512 * 1024 * 1024) {
  ...
  size_t adjust_prefill_size_internal(size_t requested_size) {
    ...
    if (mem_err == cudaSuccess) {
      if (requested_size > free_memory * 0.8) {
        size_t adjusted_size = static_cast<size_t>(free_memory * 0.7);
        std::cerr
            << "CudaMemoryPool (Internal): Warning - Requested prefill size ("
            << (requested_size / (1024.0 * 1024.0))
            << "MB) is too large. Adjusted to: "
...
class GlobalCudaMemoryPool {
 public:
  ...
  static bool enable_prefill_mode(size_t initial_size = 48 * 1024 * 1024,
                                  size_t max_size = 512 * 1024 * 1024) {
    return instance().enable_prefill_mode(initial_size, max_size);
  }

  static void disable_prefill_mode() { instance().disable_prefill_mode(); }
  static void reset_prefill_buffer() { instance().reset_prefill_buffer(); }
  ...
  static void* allocate_tagged(const std::string& tag, size_t size,
                               bool is_prefill_request = false) {
