...
template <typename T, int N> union Vec {
  float4 f4; // 实际载入 16 字节数据
  T t[N];    // 重解释为 N 个 T 类型元素
};

template <typename T, int N> union Vec_2 {
...
void launch_gemmv_scores(const T *x, const T *y, AccT *dst,
...
void launch_gqa_gemm(const Tensor<T> &Q,            // 查询张量 [batch_size, n_q_heads, seq_len, head_dim]
                     ...
                     bool transpose_K = true,       // 是否转置K (通常为true，用于注意力计算)
