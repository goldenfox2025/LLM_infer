...
void launch_gemv_scores(const T *x, const T *y, AccT *dst,
                        const int channel_size, const int channel_ratio,
                        const int row_size, const int stride_channel_x,
...
template <typename T>
void rope_with_device_offset(Tensor<T> *tensor, const size_t *d_offset,
...
void flash_attention(Tensor<T> &Q, const Tensor<T> &&K1, const Tensor<T> &&K2,
                     ...
                     Tensor<T> &att_output3, cudaStream_t stream = nullptr);
...
template <typename T>
void gather_fa_specialized_1branch(const std::vector<Tensor<T>> &inputs,
                                   Tensor<T> &output,
                                   cudaStream_t stream = nullptr);

// 特化版本的gather_fa实现 - 2分支
template <typename T>
void gather_fa_specialized_2branch(const std::vector<Tensor<T>> &inputs,
                                   Tensor<T> &output,
                                   cudaStream_t stream = nullptr);

// 特化版本的gather_fa实现 - 3分支
template <typename T>
void gather_fa_specialized_3branch(const std::vector<Tensor<T>> &inputs,
                                   Tensor<T> &output,
                                   cudaStream_t stream = nullptr);

// 特化版本的gather_fa实现 - 4分支
template <typename T>
void gather_fa_specialized_4branch(const std::vector<Tensor<T>> &inputs,
                                   Tensor<T> &output,
                                   cudaStream_t stream = nullptr);

// 特化版本的gather_fa实现 - 5分支
template <typename T>
void gather_fa_specialized_5branch(const std::vector<Tensor<T>> &inputs,
                                   Tensor<T> &output,
                                   cudaStream_t stream = nullptr);
...
template <typename T>
void dynamic_flash_attention_wrapper(Tensor<T> &Q, const Tensor<T> &total_K,
...
void gather_fa_graph_fixed(T **d_input_ptrs, Tensor<T> &output,
                           int *d_segment_info, cudaStream_t stream = nullptr);
...
float get_token_probability(const Tensor<T> &logits, int position,
                            uint32_t token_id, cudaStream_t stream = nullptr);

// 从logits中采样一个token并返回其概率
template <typename T>
std::pair<uint32_t, float> sample_with_prob(Tensor<T> &&logits,
                                            float temperature, float top_p,
                                            size_t top_k, curandState *states,
                                            cudaStream_t stream = nullptr);

// 采样函数的变体，将token和概率写入指定的GPU内存位置
template <typename T>
void sample_to_fixed_with_prob(Tensor<T> &&input, uint32_t *token_ptr,
...
template <typename T>
void sample_batch_to_fixed_with_prob(Tensor<T> &&logits, uint32_t *token_ptr,
