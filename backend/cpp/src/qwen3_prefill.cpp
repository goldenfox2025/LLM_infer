...
Tensor<T> Qwen3Model<T>::prefill_cuda(const Tensor<uint32_t> *input, KVCache<T> *kv_cache) {
    ...
    for (size_t i = 0; i < n_layers_; i++) {
        ...
        if (quant_type_ == 1) {
            ...
            operators_->matmul(&q_buf, &hidden_states, q_weight_tensor, q_bias, compute_streams_[0]);
            operators_->matmul(&k_buf, &hidden_states, k_weight_tensor, k_bias, compute_streams_[1]);
            operators_->matmul(&v_buf, &hidden_states, v_weight_tensor, v_bias, compute_streams_[2]);
        }
        ...
        cuda_OP::compute_attention_scores_prefill(q_buf_view, k_cache_view, att_scores, head_dim_);
        ...
        cuda_OP::compute_att_output_prefill(att_scores, v_cache_view, att_out_view, n_heads_, head_dim_, total_seq_len,
                                            n_kv_heads_);
    ...
    // std::cout << lm_head_weight.sizes()[0] << " " << lm_head_weight.sizes()[1] << std::endl;
