...
CudaMemoryPool& GlobalCudaMemoryPool::instance() {
  ...
  std::call_once(init_flag_, []() {
    ...
    std::atexit([]() {
      ...
      if (!driver_available) {
        std::cerr << "CUDA driver is shutting down or unavailable, skipping "
                     "detailed cleanup"
                  << std::endl;
      ...
      if (stats.active_allocations_count > 0) {
        std::cerr << "Warning: " << stats.active_allocations_count
                  << " active CUDA memory allocations (" << stats.active_bytes
                  << " bytes) at program exit." << std::endl;

        // 打印prefill模式的统计信息
        if (stats.prefill_buffer_size_bytes > 0) {
          std::cerr << "Prefill buffer: "
                    << (stats.prefill_buffer_size_bytes / (1024 * 1024))
                    << " MB total, "
                    << (stats.prefill_buffer_size_bytes / (1024 * 1024))
                    << " MB used" << std::endl;
        }
