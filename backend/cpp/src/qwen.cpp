...
Tensor<T> QwenModel<T>::forward_for_graph(const Tensor<uint32_t> *input, KVCache<T> *kv_cache, cudaStream_t stream) {
    ...
    for (size_t i = 0; i < n_layers_; i++) {
        ...
        if (quant_type_ == 0) {
            ...
            // 拷贝到固定缓冲区
            cudaMemcpyAsync(q_buf.data_ptr(), q_slice.data_ptr(), q_slice.numel() * sizeof(T), cudaMemcpyDeviceToDevice,
                            stream);
            cudaMemcpyAsync(k_buf.data_ptr(), k_slice.data_ptr(), k_slice.numel() * sizeof(T), cudaMemcpyDeviceToDevice,
                            stream);
            cudaMemcpyAsync(v_buf.data_ptr(), v_slice.data_ptr(), v_slice.numel() * sizeof(T), cudaMemcpyDeviceToDevice,
                            stream);
        } else {
            auto q_weight = get_weight(layer_prefix + "self_attn.q_proj");
            auto k_weight = get_weight(layer_prefix + "self_attn.k_proj");
...
void QwenModel<T>::update_rope_offset(size_t offset) {
    if (d_rope_offset_) {
        ...
        cudaError_t err =
            cudaMemcpyAsync(d_rope_offset_, &offset, sizeof(size_t), cudaMemcpyHostToDevice, graph_stream_);
        if (err != cudaSuccess) {
...
template <typename T>
void QwenModel<T>::prepare_next_graph_execution_async(size_t next_rope_offset, size_t next_total_seq_len, int layer_idx,
                                                      KVCache<T> *kv_cache) {
    // 异步准备下一次图执行的动态数据
...
void QwenModel<T>::save_graph_tensors_after_execution(const std::string &save_prefix) {
    ...
    cudaMemcpy(host_graph_input.data(), graph_input_tensor_.data_ptr(), graph_input_tensor_.numel() * sizeof(uint32_t),
    ...
    if (token_id < embedding_weight.sizes()[0]) {
        ...
        size_t offset = token_id * embedding_weight.sizes()[1];
        cudaMemcpy(token_embedding.data_ptr(), static_cast<const T *>(embedding_weight.data_ptr()) + offset,
                   embedding_weight.sizes()[1] * sizeof(T), cudaMemcpyDeviceToDevice);
        save_tensor_to_binary(token_embedding,
                              save_prefix + "_graph_token_" + std::to_string(token_id) + "_embedding.bin");
