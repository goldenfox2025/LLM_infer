...
__global__ void rope_kernel(T *tensor, size_t batch_size, size_t seq_len,
                            size_t n_heads, size_t head_dim, size_t offset,
                            float theta) {
  // --- 1. 确定Grid中此Block的职责 ---
...
void RopeCUDAOperator<T>::operator()(Tensor<T> *x, size_t offset, float theta,
  ...
  // --- 3. 计算Kernel启动配置 ---
  size_t head_dim_half =
      head_dim / 2;                // RoPE作用于向量的一半，总共有这么多“对”
  ...
  // --- 4. 启动Kernel ---
  rope_kernel<T, actual_pairs_per_thread><<<grid_dim, block_dim, 0, stream>>>(
      x->data_ptr(), batch_size, seq_len, n_heads, head_dim, offset, theta);
