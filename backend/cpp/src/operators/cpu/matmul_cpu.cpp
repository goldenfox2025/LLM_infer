...
template <typename T>
void MatmulCPUOperator<T>::operator()(Tensor<T>* output, Tensor<T>* input,
  ...
  if constexpr (std::is_same_v<T, __nv_bfloat16>) {
    throw std::runtime_error(
        "MatMul operator for __nv_bfloat16 not supported on CPU platform");
  } else {
    // 获取输入张量和权重张量的形状
    ...
    if (input_shape[1] != weight_shape[1]) {
      throw std::runtime_error(
          "Input and weight dimensions mismatch for MatMul");
    }
