...
InferenceEngine<T>::InferenceEngine(std::shared_ptr<BaseModel> model,
  ...
  std::cout << "【引擎初始化完成】模型当前设备: "
            << (model_device == Device::CUDA ? "CUDA" : "CPU") << std::endl;
  std::cout << "【引擎初始化完成】引擎设备: "
...
void InferenceEngine<T>::generate_with_callback(
  ...
  if (!has_warmed_up_ && device_ == Device::CUDA) {
    ...
    try {
      ...
      Tensor<uint32_t> input_tensor(std::move(warmup_input_copy),
                                    {warmup_input.size()}, device_);
