...
void InferenceEngine<T>::generate_with_callback(const std::vector<uint32_t>& input_ids, size_t max_length,
    ...
    if (!has_warmed_up_ && device_ == Device::CUDA) {
        ...
        // 创建一个小的输入序列用于预热
        std::vector<uint32_t> warmup_input(20, 1);
    ...
    std::thread generation_thread([&, this, input_ids_copy = input_ids]() {
        try {
...

            {
                ...
                // 输出计时结果，确保立即刷新输出缓冲区
                std::cout << "Prefill阶段完成，耗时: " << std::fixed << std::setprecision(2) << prefill_duration.count()
                          << " 毫秒" << std::endl
                          << std::flush;
            ...
            // 输出第一个token处理时间
            std::cout << "第一个Token处理耗时: " << std::fixed << std::setprecision(2) << token_duration.count()
                      << " 毫秒" << std::endl
                      << std::flush;
            ...
            std::cout << "总Prefill过程耗时: " << std::fixed << std::setprecision(2) << total_prefill_duration.count()
                      << " 毫秒" << std::endl
                      << std::flush;
