...
__global__ void gemv_qkv_kernel(const T *A, const T *B, T *q, T *k, T *v, const T *bias, int *C_off, int N, int K,
...
void gemv_qkv(const Tensor<T> *A, const Tensor<T> *B, Tensor<T> *q, Tensor<T> *k, Tensor<T> *v, const Tensor<T> *bias,
              int *out_offset, size_t Q_len, size_t K_len, size_t V_len, cudaStream_t stream) {
    const int N = B->sizes()[1];
    const int K = B->sizes()[0];
    // int p = -1;
    // cudaMemcpy(&p, out_offset, sizeof(int), cudaMemcpyDeviceToHost);
    // std::cout << "out_offset: " << p << std::endl;
    ...
    // 启动内核
    gemv_qkv_kernel<T>
        <<<gridDim, blockDim, 0, stream>>>(d_A, d_B, d_q, d_k, d_v, d_bias, out_offset, N, K, Q_len, K_len, V_len);
}
...
template void gemv_qkv<float>(const Tensor<float> *A, const Tensor<float> *B, Tensor<float> *q, Tensor<float> *k,
                              Tensor<float> *v, const Tensor<float> *bias, int *out_offset, size_t Q_len, size_t K_len,
                              size_t V_len, cudaStream_t stream);
