...
// CUDA Kernel for RoPE with device offset
template <typename T, int actual_pairs_per_thread = 2>
__global__ void rope_kernel_device_offset(T *tensor, size_t batch_size, size_t seq_len,
                                          size_t n_heads, size_t head_dim, const size_t* d_offset,
                                          float theta) {
  // 从设备内存读取offset
  ...
  size_t absolute_seq_pos =
      seq_idx_in_batch + offset;  // Token在原始完整序列中的绝对位置

  // 计算指向当前 (样本, 位置, 头) 的数据指针
  T *current_head_ptr =
      tensor + b_idx * seq_len * n_heads * head_dim +  // 批次偏移
  ...
  for (int i = 0; i < actual_pairs_per_thread; ++i) {
    ...
    if (rot_dim < head_dim_half) {
      ...
      current_head_ptr[rot_dim + head_dim_half] =
...
void rope_with_device_offset(Tensor<T> *tensor, const size_t* d_offset, float theta, cudaStream_t stream) {
  ...
  if (sizes.size() < 3) {
    throw std::runtime_error(
        "RoPE: Input tensor needs at least 3D (seq_len, n_heads, head_dim).");
  }
  ...
  if (head_dim_half == 0) return;  // 没有可旋转的维度对
  ...
  int threads_per_block_dim =
      (head_dim_half + actual_pairs_per_thread - 1) / actual_pairs_per_thread;
...
// 显式模板实例化
template void rope_with_device_offset<float>(Tensor<float> *tensor, const size_t* d_offset, float theta, cudaStream_t stream);
template void rope_with_device_offset<__nv_bfloat16>(Tensor<__nv_bfloat16> *tensor, const size_t* d_offset, float theta, cudaStream_t stream);
