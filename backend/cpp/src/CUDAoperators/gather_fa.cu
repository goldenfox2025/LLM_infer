...
// 优化后的 kernel：直接由每个线程读取全局 m 与 l 数据，无需使用共享内存和同步
template <typename T>
__global__ void gather_fa_kernel_3_inputs_optimized(const T* T1_ptr,
  ...
  if (std::isfinite(m2)) {
    ...
    float new_global_m =
        std::isfinite(old_global_m) ? fmaxf(old_global_m, m2) : m2;
    float exp_old = std::isfinite(old_global_m)
  ...
  if (std::isfinite(m3)) {
    ...
    float old_global_l = global_l;
    float new_global_m =
        std::isfinite(old_global_m) ? fmaxf(old_global_m, m3) : m3;
    float exp_old = std::isfinite(old_global_m)
                        ? __expf(old_global_m - new_global_m)
                        : 0.0f;
  ...
  // --- 最终归一化 ---
  float final_out =
      (global_l > 0.0f && std::isfinite(global_l)) ? global_o / global_l : 0.0f;
