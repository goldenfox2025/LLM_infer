...
// --------------------------------------------------
template <typename T>
__global__ void add_kernel(const T *A, const T *B, T *out, int total) {
...
// 显式实例化模板函数
template void add<float>(Tensor<float> *, const Tensor<float> *,
                          const Tensor<float> *);

template void add<nvbf16>(Tensor<nvbf16> *, const Tensor<nvbf16> *,
                           const Tensor<nvbf16> *);
}  // namespace cuda_OP
