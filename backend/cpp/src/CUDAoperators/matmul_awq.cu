...
__launch_bounds__(WARPS_PER_BLOCK *WARP_SIZE) __global__
  ...
  constexpr int K_PER_ITERATION =
      WARP_SIZE * VEC_SIZE;  // 每个 Warp 在内层循环处理的 K 数量

  // --- 块和线程索引 ---
  const int lane = threadIdx.x % WARP_SIZE;
  const int warp_id_in_block =
      threadIdx.x / WARP_SIZE;  // Block 内的 Warp ID (0 to WARPS_PER_BLOCK-1)
  const int block_warp_idx =
      blockIdx.x * WARPS_PER_BLOCK + warp_id_in_block;  // 全局 Warp ID

  // --- N 相关索引计算 (修改) ---
  // 每个 Warp 处理 N_PER_WARP 个输出列
  const int warp_n_base =
      block_warp_idx * N_PER_WARP;  // 这个 Warp 负责的起始 n_global
...
#pragma unroll
  ...
  __shared__ uint32_t
      smemB[WARPS_PER_BLOCK][N_PER_WARP][BK_GEMV];          // Packed 权重
...
#pragma unroll
  ...
  const int N_PACKED =
      (N + PACK_FACTOR - 1) / PACK_FACTOR;  // Total packed columns
...
#pragma unroll
    for (int load_idx = threadIdx.x * VEC_SIZE; load_idx < BK_GEMV;
      ...
      if (k_tile + VEC_SIZE <=
          BK_GEMV) {  // Ensure we don't write past smemA boundary
        if (k_global + VEC_SIZE <=
            K) {  // Check if global K is fully within bounds
...
#pragma unroll
    for (int k_tile_offset = 0; k_tile_offset < BK_GEMV;
         k_tile_offset += WARP_SIZE) {
...
#pragma unroll
          for (int n_offset = 0; n_offset < N_PER_WARP; ++n_offset) {
            ...
            if (current_n_packed >= 0) {  // Check if this n_global is valid
              smemB[warp_id_in_block][n_offset][k_tile] =
                  qwt[k_global * qwt_stride + current_n_packed];
            } else {
              smemB[warp_id_in_block][n_offset][k_tile] =
                  0;  // Pad with 0 for invalid N
            }
          }
        } else {
...
#pragma unroll
    ...
    if (lane < N_PER_WARP) {
      ...
      int current_n_packed =
          n_packed_global[lane];  // Get precomputed packed index

      // 加载 Scale
      bool valid_s = valid_group && (n_global < N);
      smemS[warp_id_in_block][lane] =
          valid_s ? scl[current_group_idx * N + n_global]
                  : static_cast<ScaleType>(
                        1.0f);  // Use 1.0f for invalid scale? Or 0? Check AWQ
                                // logic. Assume 1.0f.

      // 加载 Packed Zero-point
      bool valid_z = valid_group &&
                     (current_n_packed >= 0);  // Check if packed index is valid
      smemZ[warp_id_in_block][lane] =
          valid_z ? zos[current_group_idx * N_PACKED + current_n_packed]
                  : 0;  // Pad with 0
    }
    ...
    // --- K 块内计算 (修改) ---
    const int num_k_iterations =
        BK_GEMV / K_PER_ITERATION;  // = BK_GEMV / (WARP_SIZE * VEC_SIZE)
...
#pragma unroll
        for (int n_offset = 0; n_offset < N_PER_WARP; ++n_offset) {
          ...
          // 累加 (FMA)
          thread_sum[n_offset] =
              fmaf(current_inp_float, dequant_w, thread_sum[n_offset]);
        }  // 结束 n_offset 循环
      }  // 结束 VEC_SIZE 循环 (i)
    }  // 结束 K 块内迭代循环 (w)
...
#pragma unroll
  for (int n_offset = 0; n_offset < N_PER_WARP; ++n_offset) {
    if (n_packed_global[n_offset] >= 0) {  // Only reduce valid columns
      thread_sum[n_offset] =
          warp_reduce_sum_f32<WARP_SIZE>(thread_sum[n_offset]);
    }
  }
...
void matmul_quantized(const Tensor<T> &input, const Tensor<int32_t> &qweight,
                      const Tensor<ScaleType> &scales_input,
  ...
  if (M == 1) {  // Assuming M == 1 is the GEMV case
    constexpr int BK_GEMV =
        64;  // Or 64, must be <= group_size and multiple of K_PER_ITERATION
    constexpr int VEC_SIZE_GEMV = 2;    // Example vectorization
    constexpr int N_PER_WARP_GEMV = 8;  // <<< 每个 Warp 处理 4 个 N
    constexpr int WARPS_PER_BLOCK_GEMV =
        8;  // <<< 调整 Block 内 Warp 数量，例如 8 个 Warp (256 threads/block)
            // 你原来的 WARPS_PER_BLOCK=32 (1024 threads) 可能太大了，需要根据
            // GPU 调整

    // 确保 WARPS_PER_BLOCK * WARP_SIZE <= maxThreadsPerBlock (通常是 1024)
    static_assert(WARPS_PER_BLOCK_GEMV * WARP_SIZE <= 1024,
                  "Too many threads per block");

    // 计算需要的总 Warp 数量
    const int total_n_tasks = (N + N_PER_WARP_GEMV - 1) / N_PER_WARP_GEMV;
    // 计算 Grid 维度
    const dim3 grid_gemv((total_n_tasks + WARPS_PER_BLOCK_GEMV - 1) /
    ...
    size_t smem_size =
        BK_GEMV *
        sizeof(T);  // smemA is shared by block implicitly in declaration
    smem_size += WARPS_PER_BLOCK_GEMV * N_PER_WARP_GEMV * BK_GEMV *
                 sizeof(int32_t);  // smemB
    smem_size +=
        WARPS_PER_BLOCK_GEMV * N_PER_WARP_GEMV * sizeof(ScaleType);  // smemS
    smem_size +=
        WARPS_PER_BLOCK_GEMV * N_PER_WARP_GEMV * sizeof(int32_t);  // smemZ
    ...
    // 调用修改后的 Kernel
    awq_gemv_warp_vectorized_n<T, ScaleType, BK_GEMV, VEC_SIZE_GEMV,
                               N_PER_WARP_GEMV, WARPS_PER_BLOCK_GEMV>
...
// 原始的float scales版本
template void matmul_quantized<float, float>(const Tensor<float> &,
...
template void matmul_quantized<__nv_bfloat16, __nv_bfloat16>(
...
template void matmul_quantized<__nv_bfloat16, float>(
    const Tensor<__nv_bfloat16> &, const Tensor<int32_t> &,
    const Tensor<float> &, const Tensor<int32_t> &, int,
    Tensor<__nv_bfloat16> *, cudaStream_t, const Tensor<__nv_bfloat16> *);

// half输入和half scales版本
template void matmul_quantized<__half, __half>(const Tensor<__half> &,
                                               const Tensor<int32_t> &,
                                               const Tensor<__half> &,
                                               const Tensor<int32_t> &, int,
...
template void matmul_quantized<__half, float>(const Tensor<__half> &,
                                              const Tensor<int32_t> &,
                                              const Tensor<float> &,
                                              const Tensor<int32_t> &, int,
                                              Tensor<__half> *, cudaStream_t,
