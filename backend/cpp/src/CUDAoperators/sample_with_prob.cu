...
void sample_to_fixed_with_prob(Tensor<T>&& input, uint32_t* token_ptr, float* prob_ptr, float temperature, float top_p,
    ...
    // 使用原始sample_with_prob函数获取采样结果
    auto [sampled_token, sampled_prob] =
        sample_with_prob(std::move(input), temperature, top_p, top_k, d_states, stream);

    // 将结果复制到指定的输出位置
    cudaMemcpyAsync(token_ptr, &sampled_token, sizeof(uint32_t), cudaMemcpyHostToDevice, stream);
    cudaMemcpyAsync(prob_ptr, &sampled_prob, sizeof(float), cudaMemcpyHostToDevice, stream);
    cudaStreamSynchronize(stream);
}
...
void sample_batch_to_fixed_with_prob(Tensor<T>&& logits, uint32_t* token_ptr, float* prob_ptr, float temperature,
    ...
    // 为每个序列位置创建一个视图并调用sample_to_fixed_with_prob函数
    for (size_t i = 0; i < seq_len; i++) {
        ...
        // 调用单个token的sample_to_fixed_with_prob函数
        sample_to_fixed_with_prob(std::move(logit_view), token_ptr + i, prob_ptr + i, temperature, top_p, top_k,
                                  d_states, stream);
    }
}
