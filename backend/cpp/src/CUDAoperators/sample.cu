  ...
  if (tid == 0) {
    ...
    if (total_exp_sum <= 1e-9f || k == 0) {
      ...
      selected_final_index =
          static_cast<uint32_t>(d_sorted_topk_indices[0]);  // 默认值
      float* s_exp_vals =
          shared_storage.combined.exp_vals;  // 指向共享内存中的 exp 数组
      for (int i = 0; i < k; ++i) {
        // 从共享内存读取预先计算好的 exp 值
...
uint32_t* sample(Tensor<T>&& logits, float temperature,
  ...
  // 分配临时设备内存
  T* d_scaled_logits = static_cast<T*>(
      pool.allocate(vocab_size * sizeof(T)));  // 存储缩放后的 logits
  float* d_max_val = static_cast<float*>(
      pool.allocate(sizeof(float)));  // 存储缩放后 logits 的最大值
  int* d_indices = static_cast<int*>(
      pool.allocate(vocab_size * sizeof(int)));  // 存储原始索引
  T* d_sorted_logits = static_cast<T*>(
      pool.allocate(vocab_size * sizeof(T)));  // 存储排序后的 logits
  int* d_sorted_indices = static_cast<int*>(
      pool.allocate(vocab_size * sizeof(int)));  // 存储排序后的索引
  uint32_t* d_sampled_index;
  (cudaMalloc(&d_sampled_index, sizeof(uint32_t)));  // 存储最终采样结果
  ...
  if (sample_shared_mem > max_shared_mem_per_block) {
    throw std::runtime_error(
        "计算出的所需共享内存 (" + std::to_string(sample_shared_mem) +
        ") 超过设备限制 (" + std::to_string(max_shared_mem_per_block) +
