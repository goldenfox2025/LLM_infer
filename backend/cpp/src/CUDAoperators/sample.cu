...
// 功能: 将输入的 logits 除以 temperature 并初始化一个从 0 到 vocab_size-1 的索引数组。
template <typename T>
__global__ void scale_logits_and_init_indices_kernel(
    const T* __restrict__ logits,    // 输入: 原始 logits (设备指针)
...
__global__ void sample_from_sorted_topk_kernel(
    const T* __restrict__ d_sorted_topk_logits,    // 输入: 排序后的 Top-K logits (设备指针)
    const int* __restrict__ d_sorted_topk_indices, // 输入: 排序后的 Top-K 索引 (设备指针)
    size_t k,                                     // 输入: Top-K 中的 'k' 值 (必须 <= MAX_TOPK)
    const float* __restrict__ d_max_val_ptr,       // 输入: 所有缩放后 logits 的最大值 (设备指针)
  ...
  __syncthreads(); // 确保所有 exp_vals 都已写入共享内存，并且所有线程都完成了计算
...
// 输入:
//   - logits: 输入的 logits 张量 (T 类型, 形状 [seq_len, vocab_size], 必须在 CUDA 设备上)
...
//   - stream: CUDA 流
// 返回:
//   - 指向设备端存储最终采样结果 (单个 uint32_t 索引) 的指针。注意：结果仍在 GPU 上。
template <typename T>
uint32_t* sample(Tensor<T>&& logits, float temperature,
  ...
  if (top_k > MAX_TOPK) {
    throw std::runtime_error("请求的 top_k (" + std::to_string(top_k) +
                             ") 超过了 Kernel 2 的 MAX_TOPK 限制 (" +
  ...
  uint32_t* d_sampled_index = static_cast<uint32_t*>(pool.allocate(sizeof(uint32_t))); // 存储最终采样结果
  ...
  // 输入: d_scaled_logits, d_indices, vocab_size, d_sort_temp_storage (临时存储指针)
  // 输出: d_sorted_logits (排序后的键), d_sorted_indices (排序后的值)
  // 作用: 根据键 (logits) 对键值对 (logit, index) 进行降序排序。
  CUDA_CHECK(cub::DeviceRadixSort::SortPairsDescending(
      d_sort_temp_storage, sort_temp_storage_bytes, d_scaled_logits,
      d_sorted_logits, d_indices, d_sorted_indices, vocab_size, 0,
      sizeof(T) * 8, stream));
  ...
  size_t reduce_storage_size_est = sizeof(
      cub::BlockReduce<float, sample_block_size>::TempStorage); // CUB Reduce 存储
  size_t exp_values_size =
      MAX_TOPK * sizeof(float); // 存储 exp 值的数组大小 (基于 MAX_TOPK)
  size_t sample_shared_mem =
      reduce_storage_size_est + exp_values_size; // 总共享内存需求
  ...
  if (sample_shared_mem > max_shared_mem_per_block) {
      throw std::runtime_error("计算出的所需共享内存 (" +
