...
Qwen3Model<T>::Qwen3Model(
  ...
  if (config.find("head_dim") != config.end()) {
    ...
    std::cout << "计算head_dim: " << head_dim_
              << " (hidden_size=" << hidden_size_ << ", n_heads=" << n_heads_
              << ")" << std::endl;
  }

  // 检查并修正hidden_size与head_dim*n_heads的不一致
  size_t expected_hidden_size = head_dim_ * n_heads_;
  if (hidden_size_ != expected_hidden_size) {
    std::cout << "警告: hidden_size (" << hidden_size_
              << ") 与 head_dim*n_heads (" << expected_hidden_size << ") 不匹配"
              << std::endl;
    std::cout << "自动调整 hidden_size 为 " << expected_hidden_size
              << std::endl;
    hidden_size_ = expected_hidden_size;
  }
