...
static void verify_matrix_layout(const Tensor<float>& a, const Tensor<float>& b, size_t M, size_t K, size_t N) {
...
// 比较两个张量并报告精度指标
static bool compare_and_report(const Tensor<float>& reference, const Tensor<float>& test, const std::string& test_name,
...
// 对单个矩阵乘法函数进行基准测试
static void benchmark_gemm(const std::string& name, GemmFunc func, const Tensor<float>& A, const Tensor<float>& B,
                           const Tensor<float>& reference, size_t B_batch, size_t M, size_t K, size_t N, int warmup,
                           int iters) {
    std::cout << "\n=== " << name << " ===" << std::endl;
...
int main(int argc, char** argv) {
    ...
    register_gemm("avx_v4", avx_OP::matmul_v7, "V2 + 顺序8x16 + OpenMP (137+ GFLOP/s)");
    ...
    for (const auto& [name, gflops] : results) {
        ...
        std::cout << std::setw(12) << name << ": " << std::setw(8) << gflops << " GFLOP/s  (speedup: " << std::setw(6)
                  << speedup << "x)\n";
    }
