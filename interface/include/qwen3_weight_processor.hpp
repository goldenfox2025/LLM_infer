...
inline void process_global_weights_bf16(
  ...
  for (const auto& [src_key, dst_key] : global_weights_map) {
    if (weights.contains(src_key)) {
      ...
      // 计算参数数量并累加到总数
      size_t params_count =
          weight_processor_utils::calculate_params_count(tensor);

      Tensor<__nv_bfloat16> bf16_tensor =
          weight_processor_utils::convert_bf16_tensor(tensor);
      if (dst_key == "lm_head") {
  ...
  for (auto item : weights) {
    ...
    if (key.find("model.layers.") == 0) {
      ...
      if (key.find(".qweight") != std::string::npos) {
        ...
        std::string dst_key = dst_prefix + std::to_string(layer);
        weight_processor_utils::print_processing_info(key,
                                                      dst_key + ".qweight");
      ...
      // 处理非量化层级权重
        ...
        } else if (key.find("post_attention_layernorm.weight") !=
                   std::string::npos) {
