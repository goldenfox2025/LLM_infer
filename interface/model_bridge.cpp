...
bool init_model(py::dict config, py::dict weights, const std::string& model_type) {
    try {
        // 打印配置与权重调试信息
        ...
        if (model_type == "llama") {
            result = ModelInitializer::init_llama_model(config, weights, g_model, g_engine);
        } else if (model_type == "qwen") {
            result = ModelInitializer::init_qwen_fp32_model(config, weights, g_model, g_engine);
        } else if (model_type == "qwen_bf16") {
            result = ModelInitializer::init_qwen_bf16_model(config, weights, g_model, g_engine);
        } else if (model_type == "qwen_awq") {
            result = ModelInitializer::init_qwen_awq_model(config, weights, g_model, g_engine);
        } else if (model_type == "qwen3_bf16") {
            result = ModelInitializer::init_qwen3_bf16_model(config, weights, g_model, g_engine);
        } else if (model_type == "qwen3_awq") {
            result = ModelInitializer::init_qwen3_awq_model(config, weights, g_model, g_engine);
        } else {
        ...
        if (result) {
            ...
            // 构建基础配置用于初始化CUDA内存池
            std::unordered_map<std::string, int> cpp_config = ModelInitializer::build_base_config(config);
...
void generate_text_stream(const std::vector<uint32_t>& input_ids, py::function callback, size_t max_length = 100,
                          float temperature = 1.0f, float top_p = 0.9f, size_t top_k = 50) {
    if (!g_engine) {
        throw std::runtime_error("Model not initialized");
    }

    g_engine->generate_with_callback(input_ids, max_length, temperature, top_p, top_k, [callback](uint32_t token) {
...
bool set_default_device(const std::string& device_str) {
    try {
        ...
        if (device_str == "cuda" || device_str == "CUDA") {
            ...
            std::cerr << "Invalid device: " << device_str << ". Valid options are 'cuda' or 'cpu'." << std::endl;
            return false;
        }

        DeviceManager::instance().setDefaultDevice(device);
        std::cout << "Default device set to: " << (device == Device::CUDA ? "CUDA" : "CPU") << std::endl;
        return true;
    } catch (const std::exception& e) {
...
PYBIND11_MODULE(model_bridge, m) {
    m.def("init_model", &init_model, py::arg("config"), py::arg("weights"), py::arg("model_type") = "llama",
          "Initialize and verify the model");
    m.def("generate_text_stream", &generate_text_stream, py::arg("input_ids"), py::arg("callback"),
          py::arg("max_length") = 100, py::arg("temperature") = 1.0f, py::arg("top_p") = 0.9f, py::arg("top_k") = 50,
          "Stream generated tokens via callback");
    m.def("set_default_device", &set_default_device, py::arg("device"),
          "Set the default device for model execution (cuda or cpu)");
    m.def("get_default_device", &get_default_device, "Get the current default device for model execution");
}
