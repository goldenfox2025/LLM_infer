...
Qwen3WeightProcessor::process_weights_bf16(const py::dict& weights) {
  std::unordered_map<std::string, Tensor<__nv_bfloat16>> cpp_weights;

  // 初始化进度条
  size_t total_weights = weights.size();
  init_progress(total_weights, "Qwen3 BF16");
...
std::tuple<std::unordered_map<std::string, Tensor<__nv_bfloat16>>,
           ...
           std::unordered_map<std::string, Tensor<__nv_bfloat16>>,
           std::unordered_map<std::string, Tensor<int32_t>>>
Qwen3WeightProcessor::process_weights_awq(const py::dict& weights) {
  std::unordered_map<std::string, Tensor<__nv_bfloat16>> cpp_weights;
  ...
  // 初始化进度条
  size_t total_weights = weights.size();
  init_progress(total_weights, "Qwen3 AWQ");
...
// 处理 BF16 全局权重
void Qwen3WeightProcessor::process_global_weights_bf16(
...
// 处理 BF16 层级权重
void Qwen3WeightProcessor::process_layer_weights_bf16(
    const py::dict& weights,
    std::unordered_map<std::string, Tensor<__nv_bfloat16>>& cpp_weights) {
  // 层级权重映射
...
// 处理 AWQ 全局权重
void Qwen3WeightProcessor::process_global_weights_awq(
    const py::dict& weights,
    std::unordered_map<std::string, Tensor<__nv_bfloat16>>& cpp_weights) {
  // 全局权重映射
  const std::unordered_map<std::string, std::string> global_weights_map = {
...
// 处理 AWQ 量化权重
void Qwen3WeightProcessor::process_quantized_weights_awq(
    const py::dict& weights,
    std::unordered_map<std::string, Tensor<__nv_bfloat16>>& cpp_weights,
    std::unordered_map<std::string, Tensor<int32_t>>& cpp_qweight_params,
    std::unordered_map<std::string, Tensor<__nv_bfloat16>>& cpp_scales_params,
  ...
  for (auto item : weights) {
    ...
    if (key.find("model.layers.") == 0) {
      ...
      if (key.find(".qweight") != std::string::npos) {
        ...
        std::string dst_key = dst_prefix + std::to_string(layer);
        print_processing_info(key, dst_key + ".qweight");
      ...
      // 处理缩放因子
        ...
        std::string dst_key = dst_prefix + std::to_string(layer);
        print_processing_info(key, dst_key + ".scales");
      ...
      // 处理零点
        ...
        std::string dst_key = dst_prefix + std::to_string(layer);
        print_processing_info(key, dst_key + ".qzeros");
      ...
      // 处理非量化层级权重
               ...
               key.find("post_attention_layernorm.weight") !=
                   std::string::npos) {
        // 提取层索引
        size_t start = std::string("model.layers.").size();
        size_t end = key.find('.', start);
        ...
        if (key.find("input_layernorm.weight") != std::string::npos) {
          dst_prefix = "rms_att_w";
        } else {  // post_attention_layernorm.weight
          dst_prefix = "rms_ffn_w";
        }
