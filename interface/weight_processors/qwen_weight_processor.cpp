...
QwenWeightProcessor::process_weights_fp32(const py::dict& weights) {
  std::unordered_map<std::string, Tensor<float>> cpp_weights;

  // 初始化进度条
  size_t total_weights = weights.size();
  init_progress(total_weights, "Qwen FP32");
...
QwenWeightProcessor::process_weights_bf16(const py::dict& weights) {
  std::unordered_map<std::string, Tensor<__nv_bfloat16>> cpp_weights;

  // 初始化进度条
  size_t total_weights = weights.size();
  init_progress(total_weights, "Qwen BF16");
...
std::tuple<std::unordered_map<std::string, Tensor<__nv_bfloat16>>,
           ...
           std::unordered_map<std::string, Tensor<__nv_bfloat16>>,
           std::unordered_map<std::string, Tensor<int32_t>>>
QwenWeightProcessor::process_weights_awq(const py::dict& weights) {
  std::unordered_map<std::string, Tensor<__nv_bfloat16>> cpp_weights;
  ...
  // 初始化进度条
  size_t total_weights = weights.size();
  init_progress(total_weights, "Qwen AWQ");
...
// 处理 FP32 全局权重
void QwenWeightProcessor::process_global_weights_fp32(
  ...
  if (weights.contains("lm_head.weight")) {
    ...
    cpp_weights.emplace(
        "lm_head", Tensor<float>(std::move(data), shape).transpose(-1, -2));
  }
}

// 处理 FP32 层级权重
void QwenWeightProcessor::process_layer_weights_fp32(
    const py::dict& weights,
    std::unordered_map<std::string, Tensor<float>>& cpp_weights) {
  // 层级权重映射
...
// 处理 BF16 全局权重
void QwenWeightProcessor::process_global_weights_bf16(
    const py::dict& weights,
    std::unordered_map<std::string, Tensor<__nv_bfloat16>>& cpp_weights) {
  // 全局权重映射表
...
// 处理 BF16 层级权重
void QwenWeightProcessor::process_layer_weights_bf16(
    const py::dict& weights,
    std::unordered_map<std::string, Tensor<__nv_bfloat16>>& cpp_weights) {
  // 层级权重映射
  const std::vector<std::pair<std::string, std::string>>
...
// 处理 AWQ 全局权重
void QwenWeightProcessor::process_global_weights_awq(
    const py::dict& weights,
    std::unordered_map<std::string, Tensor<__nv_bfloat16>>& cpp_weights) {
  // 全局权重映射表（非量化权重）
  const std::unordered_map<std::string, std::string> global_weights_map = {
...
// 处理 AWQ 量化权重
void QwenWeightProcessor::process_quantized_weights_awq(
    const py::dict& weights,
    std::unordered_map<std::string, Tensor<__nv_bfloat16>>& cpp_weights,
    std::unordered_map<std::string, Tensor<int32_t>>& cpp_qweight_params,
    std::unordered_map<std::string, Tensor<__nv_bfloat16>>& cpp_scales_params,
  ...
  for (auto item : weights) {
    ...
    if (key.find(".qweight") != std::string::npos) {
      ...
      print_processing_info(key, full_dst_key);
