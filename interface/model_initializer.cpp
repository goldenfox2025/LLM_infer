...
bool ModelInitializer::init_llama_model(py::dict config, py::dict weights, 
    ...
    try {
        ...
        // 处理 Llama 权重（FP32）
        auto cpp_weights_fp32 = WeightProcessor::process_llama_weights(weights);
...
// 初始化 Qwen FP32 模型
bool ModelInitializer::init_qwen_fp32_model(py::dict config, py::dict weights, 
                                          std::shared_ptr<BaseModel>& model,
                                          std::unique_ptr<infer_base>& engine) {
    try {
        // 构建配置
        ...
        // 处理 Qwen FP32 权重
        auto cpp_weights_fp32 = WeightProcessor::process_qwen_weights_fp32(weights);
...
// 初始化 Qwen BF16 模型
bool ModelInitializer::init_qwen_bf16_model(py::dict config, py::dict weights, 
                                          std::shared_ptr<BaseModel>& model,
                                          std::unique_ptr<infer_base>& engine) {
    try {
        // 构建配置
        ...
        auto cpp_weights_bf16 = WeightProcessor::process_qwen_weights_bf16(weights);
...
// 初始化 Qwen AWQ 模型
bool ModelInitializer::init_qwen_awq_model(py::dict config, py::dict weights, 
                                         std::shared_ptr<BaseModel>& model,
                                         std::unique_ptr<infer_base>& engine) {
    try {
        // 构建配置
        ...
        auto [bf16_weights, qweight_params, scales_params, qzeros_params] =
            WeightProcessor::process_qwen_weights_awq(weights);
        ...
        model = ModelFactory::create_model_quantized(
            ModelType::QWEN_AWQ, bf16_weights, qweight_params, scales_params, qzeros_params,
            cpp_config);
