...
// 构建基础配置
std::unordered_map<std::string, int> ModelInitializer::build_base_config(py::dict config) {
...
bool ModelInitializer::init_llama_model(py::dict config, py::dict weights, std::shared_ptr<BaseModel>& model,
    ...
    try {
        ...
        cpp_config["num_attention_heads"] = config["num_attention_heads"].cast<int>();
        cpp_config["num_key_value_heads"] = config["num_key_value_heads"].cast<int>();
        cpp_config["intermediate_size"] = config["intermediate_size"].cast<int>();
        cpp_config["rms_norm_eps"] = static_cast<int>(config["rms_norm_eps"].cast<float>());
        cpp_config["rope_theta"] = static_cast<int>(config["rope_theta"].cast<float>());
        ...
        model = ModelFactory::create_model(ModelType::LLAMA, cpp_weights_fp32, cpp_config);
...
bool ModelInitializer::init_qwen_fp32_model(py::dict config, py::dict weights, std::shared_ptr<BaseModel>& model,
    ...
    try {
        ...
        // 处理 Qwen FP32 权重
        auto cpp_weights_fp32 = weight_processor::process_qwen_weights_fp32(weights);
        model = ModelFactory::create_model(ModelType::QWEN, cpp_weights_fp32, cpp_config);
        ...
        if (default_device == Device::CPU) {
            std::cerr << "Warning: Qwen model currently only supports CUDA execution. "
        ...
        std::cerr << "Error initializing Qwen FP32 model: " << e.what() << std::endl;
        return false;
    }
}
...
bool ModelInitializer::init_qwen_bf16_model(py::dict config, py::dict weights, std::shared_ptr<BaseModel>& model,
    ...
    try {
        ...
        auto cpp_weights_bf16 = weight_processor::process_qwen_weights_bf16(weights);
        ...
        // Qwen 模型目前仅支持 CUDA，如果默认设备是CPU，发出警告并强制使用CUDA
        if (default_device == Device::CPU) {
            std::cerr << "Warning: Qwen BF16 model currently only supports CUDA execution. "
                      << "Forcing CUDA device despite CPU being requested." << std::endl;
            default_device = Device::CUDA;
        }

        model = ModelFactory::create_model_bf16(ModelType::QWEN_BF16, cpp_weights_bf16, cpp_config);
        engine = std::make_unique<InferenceEngine<__nv_bfloat16>>(model, default_device);
        model->cuda();
        return true;
    } catch (const std::exception& e) {
        std::cerr << "Error initializing Qwen BF16 model: " << e.what() << std::endl;
        return false;
    }
}
...
bool ModelInitializer::init_qwen_awq_model(py::dict config, py::dict weights, std::shared_ptr<BaseModel>& model,
    ...
    try {
        ...
        // Qwen 模型目前仅支持 CUDA，如果默认设备是CPU，发出警告并强制使用CUDA
        if (default_device == Device::CPU) {
            std::cerr << "Warning: Qwen AWQ model currently only supports CUDA execution. "
                      << "Forcing CUDA device despite CPU being requested." << std::endl;
            default_device = Device::CUDA;
        }

        // 创建带量化参数的模型
        model = ModelFactory::create_model_quantized(ModelType::QWEN_AWQ, bf16_weights, qweight_params, scales_params,
...
bool ModelInitializer::init_qwen3_bf16_model(py::dict config, py::dict weights, std::shared_ptr<BaseModel>& model,
    ...
    try {
        ...
        auto cpp_weights_bf16 = weight_processor::process_qwen3_weights_bf16(weights);
        ...
        model = ModelFactory::create_model_bf16(ModelType::QWEN3_BF16, cpp_weights_bf16, cpp_config);
        engine = std::make_unique<InferenceEngine<__nv_bfloat16>>(model, default_device);
        model->cuda();
        return true;
    } catch (const std::exception& e) {
        std::cerr << "Error initializing Qwen3 BF16 model: " << e.what() << std::endl;
        return false;
    }
}
...
bool ModelInitializer::init_qwen3_awq_model(py::dict config, py::dict weights, std::shared_ptr<BaseModel>& model,
    ...
    try {
        ...
        // Qwen3 模型目前仅支持 CUDA，如果默认设备是CPU，发出警告并强制使用CUDA
        if (default_device == Device::CPU) {
            std::cerr << "Warning: Qwen3 AWQ model currently only supports CUDA execution. "
                      << "Forcing CUDA device despite CPU being requested." << std::endl;
            default_device = Device::CUDA;
        }

        // 创建带量化参数的模型
        model = ModelFactory::create_model_quantized(ModelType::QWEN3_AWQ, bf16_weights, qweight_params, scales_params,
                                                     qzeros_params, cpp_config);
...
// 初始化 CUDA 内存池
bool ModelInitializer::init_cuda_memory_pool(const std::unordered_map<std::string, int>& config) {
    try {
        // 获取模型配置参数
